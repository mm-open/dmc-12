...
Added in v0.6 — additive and non-breaking. See [`SPEC.md`](./SPEC.md) §16.
...
Added in v0.5 — additive for OTD-omitting payloads; the one optional `out_the_door` field changes shape (field-access compatible — `amount`/`currency` stay at the top level). See [`SPEC.md`](./SPEC.md) §15.
